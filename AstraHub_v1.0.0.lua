...
local function finalGhostDeduction()
    ...
    if directType then
        ...
        return { method = "direct", name = directType, ability = "Check ghost database.", evidence = {} }
    end
...
-- Tracks whether we already sent a "GHOST IDENTIFIED" notification this session
...
local function autoPlayTick(statusLbl, resultLbl)
    ...
    -- Update status label
    pcall(function()
        statusLbl:Set("🤖 Auto Play: " .. foundCount .. "/8 evidence found")
    end)

    -- Notify newly found evidence
    if #newFound > 0 then
        Rayfield:Notify({
    ...
    if #possible == 1 then
        local g = possible[1]
        pcall(function()
            resultLbl:Set("✅ Ghost: " .. g.name)
        end)
        -- Notify identification only when narrowed to 1 for the first time
    ...
    elseif #possible > 1 then
        -- Reset flag so it fires again when narrowed to 1
        apAlreadyIdentified = false
        local names = {}
        for _, g in ipairs(possible) do table.insert(names, g.name) end
        pcall(function()
            resultLbl:Set("Possible (" .. #possible .. "): " .. table.concat(names, ", "))
        end)
    elseif foundCount > 0 then
        apAlreadyIdentified = false
        pcall(function()
            resultLbl:Set("⚠️ No match — check evidence inputs")
        end)
    else
        pcall(function()
            resultLbl:Set("Scanning for evidence...")
        end)
    end
end
...
EvidTab:CreateButton({
    ...
    Callback = function()
        ...
        if #possible == 0 then
            ...
            Rayfield:Notify({ Title = "❌ No Match", Content = "No ghost has all selected evidence.\nCheck your inputs.", Duration = 5 })
            return
        end
        local names = {}
        for _, g in ipairs(possible) do table.insert(names, g.name) end
...
EvidTab:CreateLabel("Automatically scans the game and detects evidence.")
EvidTab:CreateLabel("Temp <= 2C, EMF 5, Handprints, Orbs and more.")

local apStatusLabel = EvidTab:CreateLabel("🤖 Auto Play: Off")
local apResultLabel = EvidTab:CreateLabel("Result: —")

EvidTab:CreateToggle({
    Name         = "🤖 Auto Play (auto evidence detection)",
    CurrentValue = false,
    Flag         = "AutoPlay",
    Callback     = function(val)
        State.AutoPlay = val
        ...
        if val then
            ...
            Rayfield:Notify({
                Title    = "🤖 Auto Play ON",
                Content  = "Scanning workspace every 2 seconds.\nEvidence will be detected automatically.",
                Duration = 5,
                Image    = 4483362458,
            })
            ...
            Rayfield:Notify({
                Title    = "🤖 Auto Play OFF",
                Content  = "Evidence scanning stopped.",
                Duration = 3,
                Image    = 4483362458,
            })
        end
...
EvidTab:CreateButton({
    Name     = "🔍 Scan Once (manual)",
    Callback = function()
        Rayfield:Notify({ Title = "🤖 Scanning...", Content = "Scanning workspace...", Duration = 2 })
        task.spawn(function()
            task.wait(0.1)
            pcall(autoPlayTick, apStatusLabel, apResultLabel)
        end)
...
ESPTab:CreateSlider({
    Name = "Fill Transparency", Range = {0, 10}, Increment = 1, Suffix = "/10", CurrentValue = 4,
    Flag = "ESPFillTransp",
    Callback = function(val)
        local t = val / 10
...
ESPTab:CreateSlider({
    Name = "Outline Transparency", Range = {0, 10}, Increment = 1, Suffix = "/10", CurrentValue = 0,
...
MiscTab:CreateLabel("Auto Play: auto evidence detection")
