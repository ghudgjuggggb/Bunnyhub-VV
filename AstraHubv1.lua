...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "Demonology Edition v3.0",
    ConfigurationSaving = {
        Enabled    = true,
        FolderName = "AstraHub",
        FileName   = "Demonology",
...
local function scanItemESP()
    for _, obj in ipairs(workspace:GetDescendants()) do
        ...
        if (obj:IsA("Model") or obj:IsA("Tool") or obj:IsA("BasePart")) and obj.Parent then
            ...
            if ok then
                addESP(obj, Color3.fromRGB(0, 200, 100), Color3.fromRGB(0, 255, 150), "📦 " .. (kw or "Item"))
            end
        end
    end
end
...
local function detectHunt()
    ...
    if ghost then
        for k, v in pairs(ghost:GetAttributes()) do
            local kl = k:lower()
            if (kl:find("hunt") or kl:find("active") or kl:find("phase")) then
...
local function antiSexEscape()
    ...
    local ghost = findGhost()
    if ghost then
        local ghostPart = ghost:IsA("BasePart") and ghost or ghost:FindFirstChildWhichIsA("BasePart", true)
        if ghostPart then
...
local function tryReadGhostTypeDirectly()
    ...
    if typeAttr and type(typeAttr) == "string" and typeAttr ~= "" then return typeAttr end
    for _, v in ipairs(ghost:GetDescendants()) do
        if v:IsA("StringValue") then
            local nl = v.Name:lower()
            if nl:find("type") or nl:find("ghost") or nl:find("name") then
                if v.Value and v.Value ~= "" then return v.Value end
            end
        end
    end
    return nil
...
local function finalGhostDeduction()
    ...
    if directType then
        for _, g in ipairs(GHOST_DATA) do
            if g.name:lower() == directType:lower() then
                return { method = "direct", name = g.name, ability = g.ability, evidence = g.evidence }
            end
        end
    ...
    if #possible == 1 then
        return { method = "evidence", name = possible[1].name, ability = possible[1].ability, evidence = possible[1].evidence }
    elseif #possible > 1 then
        local names = {}
        for _, g in ipairs(possible) do table.insert(names, g.name) end
        return { method = "partial", names = names, possible = possible }
    end
    return nil
end

-- ============================================================
--  AUTO PLAY — Evidence Detection Engine
-- ============================================================

-- Checks ghost + workspace attributes for a numeric value ≤ threshold
local function ap_scanNumericAttr(objList, keyPatterns, threshold)
    for _, obj in ipairs(objList) do
        if isPlayerPart(obj) then continue end
        -- Instance values
        if obj:IsA("NumberValue") or obj:IsA("IntValue") then
            local nl = obj.Name:lower()
            for _, pat in ipairs(keyPatterns) do
                if nl:find(pat) then
                    if threshold then
                        if obj.Value <= threshold then return true end
                    else
                        if obj.Value > 0 then return true end
                    end
                end
            end
        end
        -- Attributes on Models / Parts
        ...
        if obj:IsA("BoolValue") then
            local nl = obj.Name:lower()
            for _, pat in ipairs(keyPatterns) do
                if nl:find(pat) and obj.Value then return true end
            end
        end
    end
    return false
...
-- Checks if any BasePart/Model with namePatterns exists in workspace (not player)
local function ap_objectExists(objList, namePatterns)
    for _, obj in ipairs(objList) do
        if isPlayerPart(obj) then continue end
        if obj:IsA("BasePart") or obj:IsA("Model") or obj:IsA("ParticleEmitter") then
            local nl = obj.Name:lower()
            for _, pat in ipairs(namePatterns) do
                if nl:find(pat) then return true end
            end
        end
        -- also check bool flags
...
local function ap_checkFreezing(allObjs)
    -- temperature value ≤ 2 (near freezing / sub-zero)
    local ghost = findGhost()
    if ghost then
        for k, v in pairs(ghost:GetAttributes()) do
            local kl = k:lower()
            if (kl:find("temp") or kl:find("temperature")) and type(v) == "number" then
        ...
        for _, child in ipairs(ghost:GetDescendants()) do
            if (child:IsA("NumberValue") or child:IsA("IntValue")) then
                ...
                if (nl:find("temp") or nl:find("temperature")) and child.Value <= 2 and child.Value >= -50 then
                    return true
                end
            end
        end
    end
    return ap_scanNumericAttr(allObjs, {"temp","temperature"}, 2)
end

local function ap_checkEMF5(allObjs)
    -- EMF value == 5, or any bool/attr "EMF5"/"EMFLevel5" = true
    local ghost = findGhost()
    if ghost then
        for k, v in pairs(ghost:GetAttributes()) do
            local kl = k:lower()
    ...
    for _, obj in ipairs(allObjs) do
        if isPlayerPart(obj) then continue end
        local nl = obj.Name:lower()
        -- exact EMF5 object present
        if nl == "emflevel5" or nl == "emf5" or nl == "emfreading5" then
            if obj:IsA("BasePart") or obj:IsA("Model") then return true end
            if obj:IsA("BoolValue") and obj.Value then return true end
        end
        -- NumberValue named "emf" with value ≥ 5
        if nl:find("emf") and (obj:IsA("NumberValue") or obj:IsA("IntValue")) then
            if obj.Value >= 5 then return true end
        end
        -- Attribute "Level" or "Value" ≥ 5 on EMF model
        if nl:find("emf") and (obj:IsA("BasePart") or obj:IsA("Model")) then
            for k, v in pairs(obj:GetAttributes()) do
                local kl = k:lower()
                if (kl:find("level") or kl:find("value") or kl:find("reading")) and type(v) == "number" and v >= 5 then
                    return true
                end
            end
        end
...
local function ap_checkHandprints(allObjs)
    return ap_objectExists(allObjs, {"handprint","fingerprint","uvtrace","handmark","uvmark"})
end
...
local function ap_checkSpiritBox(allObjs)
    for _, obj in ipairs(allObjs) do
        ...
        if nl:find("spiritbox") or nl:find("spirit_box") then
            ...
            -- Check active/response attributes
            if obj:IsA("BasePart") or obj:IsA("Model") then
                for k, v in pairs(obj:GetAttributes()) do
                    local kl = k:lower()
                    if kl:find("response") or kl:find("active") or kl:find("talking") then
                        if (type(v) == "boolean" and v) or (type(v) == "number" and v > 0) then return true end
                    end
                end
            end
        end
...
local function ap_checkGhostOrb(allObjs)
    for _, obj in ipairs(allObjs) do
        if isPlayerPart(obj) then continue end
        local nl = obj.Name:lower()
        if nl == "ghostorb" or nl == "ghost_orb" or nl == "orb" or nl == "ghostball" then
            if obj:IsA("BasePart") or obj:IsA("Model") or obj:IsA("ParticleEmitter") then return true end
            if obj:IsA("BoolValue") and obj.Value then return true end
        end
        -- Some games use a SpotLight or PointLight named "Orb"
        if nl == "orb" and (obj:IsA("SpotLight") or obj:IsA("PointLight")) then return true end
    end
    return false
end

local function ap_checkGhostWriting(allObjs)
    return ap_objectExists(allObjs, {"ghostwriting","ghost_writing","writingmark","inkmark"})
end

local function ap_checkLaser(allObjs)
    return ap_objectExists(allObjs, {"laserdot","laserprojecter","laserprojector","lidardot","lidar_dot","projectormark"})
end

local function ap_checkWither(allObjs)
    return ap_objectExists(allObjs, {"wither","withertrace","withermark","wither_mark","witherdecay"})
end

-- Master check table — order matters for notification readability
local AP_CHECKS = {
...
-- Called every tick of Auto Play loop
-- statusLbl and resultLbl are Rayfield label objects to update
local function autoPlayTick(statusLbl, resultLbl)
    -- Build snapshot of all workspace descendants once per tick (performance)
    local allObjs = workspace:GetDescendants()

    local newFound = {}

    for _, check in ipairs(AP_CHECKS) do
        local success, result = pcall(check.fn, allObjs)
        if success and result then
            if not EvidenceFound[check.evidence] then
    ...
    -- Count total found
    local foundList = {}
    for ev, v in pairs(EvidenceFound) do
        if v then table.insert(foundList, ev) end
    end
    local foundCount = #foundList

    -- Update status label
    pcall(function()
        statusLbl:Set("🤖 Auto Play: " .. foundCount .. "/8 улик найдено")
    end)

    -- Notify about newly found evidence
    if #newFound > 0 then
        Rayfield:Notify({
            Title    = "🤖 Новая улика!",
            Content  = table.concat(newFound, "\n"),
            Duration = 5,
            Image    = 4483362458,
        })
    end

    -- Run deduction every tick
    local _, possible = deduceGhosts()

    if #possible == 1 then
        local g = possible[1]
        pcall(function()
            resultLbl:Set("✅ Призрак: " .. g.name)
        end)
        -- Notify identification only once (when we first narrow to 1)
        if #newFound > 0 then
            Rayfield:Notify({
                Title    = "✅ ПРИЗРАК: " .. g.name:upper(),
    ...
    elseif #possible > 1 then
        ...
        for _, g in ipairs(possible) do table.insert(names, g.name) end
        pcall(function()
            resultLbl:Set("Возможно (" .. #possible .. "): " .. table.concat(names, ", "))
        end)
    elseif foundCount > 0 then
        pcall(function()
            resultLbl:Set("⚠️ Нет совпадений — проверь улики")
        end)
    else
        pcall(function()
            resultLbl:Set("Ищу улики...")
        end)
    end
end
...
-- ============================================================
--  TAB 2 — EVIDENCE (Checklist + Auto Play)
-- ============================================================
...
EvidTab:CreateButton({
    ...
    Callback = function()
        ...
        if #possible == 0 then
            ...
            Rayfield:Notify({ Title = "❌ No Match", Content = "No ghost has all selected evidence.", Duration = 5 })
            return
        end
        local names = {}
        for _, g in ipairs(possible) do table.insert(names, g.name) end
...
EvidTab:CreateLabel("Автоматически сканирует игру и находит улики.")
EvidTab:CreateLabel("Температура ≤ 2°C, EMF 5, Handprints, Orbs и др.")

local apStatusLabel = EvidTab:CreateLabel("🤖 Auto Play: выключен")
local apResultLabel = EvidTab:CreateLabel("Результат: —")

EvidTab:CreateToggle({
    Name         = "🤖 Auto Play (авто-сбор улик)",
    CurrentValue = false,
    Flag         = "AutoPlay",
    Callback     = function(val)
        State.AutoPlay = val
        ...
        if val then
            ...
            for ev in pairs(EvidenceFound) do EvidenceFound[ev] = false end
            apStatusLabel:Set("🤖 Auto Play: запущен...")
            apResultLabel:Set("Ищу улики...")

            Rayfield:Notify({
                Title    = "🤖 Auto Play ВКЛЮЧЁН",
                Content  = "Сканирую workspace каждые 2 сек.\nУлики будут определены автоматически.",
                Duration = 5,
                Image    = 4483362458,
            })
            ...
            task.spawn(function()
                while State.AutoPlay do
                    ...
                    if State.AutoPlay then
                        pcall(autoPlayTick, apStatusLabel, apResultLabel)
                    end
                end
                pcall(function() apStatusLabel:Set("🤖 Auto Play: выключен") end)
                pcall(function() apResultLabel:Set("Результат: —") end)
            end)

        else
            Rayfield:Notify({
                Title    = "🤖 Auto Play ВЫКЛЮЧЁН",
                Content  = "Сбор улик остановлен.",
                Duration = 3,
                Image    = 4483362458,
            })
        end
...
EvidTab:CreateButton({
    Name     = "🔍 Scan Once (ручной запуск)",
    Callback = function()
        Rayfield:Notify({ Title = "🤖 Scanning...", Content = "Сканирую workspace...", Duration = 2 })
        task.spawn(function()
            task.wait(0.1)
            pcall(autoPlayTick, apStatusLabel, apResultLabel)
        end)
...
EvidTab:CreateButton({
    Name     = "🗑️ Reset Auto Play",
    Callback = function()
        State.AutoPlay = false
        for ev in pairs(EvidenceFound) do EvidenceFound[ev] = false end
        apStatusLabel:Set("🤖 Auto Play: выключен")
        apResultLabel:Set("Результат: —")
        Rayfield:Notify({ Title = "Auto Play Reset", Content = "Все улики сброшены.", Duration = 2 })
    end,
})
...
ESPTab:CreateToggle({
    ...
    Callback     = function(val)
        ...
        else clearESPByCategory("GHOST") end
    end,
})
...
ESPTab:CreateToggle({
    ...
    Callback     = function(val)
        ...
        else clearESPByCategory("Evidence") end
    end,
})
...
ESPTab:CreateToggle({
    ...
    Callback     = function(val)
        ...
        else clearESPByCategory("print") end
    end,
})
...
ESPTab:CreateToggle({
    ...
    Callback     = function(val)
        State.ItemESP = val
        if val then pcall(scanItemESP); startESPLoop("ItemESP", scanItemESP, 2) end
    end,
})
...
ESPTab:CreateSlider({
    ...
    Range        = {0, 10}, Increment = 1, Suffix = "/10", CurrentValue = 4,
    Flag         = "ESPFillTransp",
    Callback     = function(val)
        local t = val / 10
...
ESPTab:CreateSlider({
    ...
    Range        = {0, 10}, Increment = 1, Suffix = "/10", CurrentValue = 0,
...
PlayerTab:CreateSlider({
    Name = "Walk Speed", Range = {8, 120}, Increment = 2, Suffix = " studs/s", CurrentValue = 16,
    Flag = "WalkSpeed",
    Callback = function(val)
        local char = LocalPlayer.Character
        if char then
...
PlayerTab:CreateSlider({
    Name = "Jump Power", Range = {50, 300}, Increment = 10, Suffix = "", CurrentValue = 50,
    Flag = "JumpPower",
    Callback = function(val)
        local char = LocalPlayer.Character
        if char then
...
PlayerTab:CreateToggle({
    Name = "✈️ Fly", CurrentValue = false, Flag = "FlyToggle",
    Callback = function(val)
        State.Fly = val
        if val then
            local char = LocalPlayer.Character
...
PlayerTab:CreateToggle({
    Name = "♾️ Infinite Stamina / Energy", CurrentValue = false, Flag = "InfStamina",
    Callback = function(val)
                    ...
                    if char then
                        ...
                        if hum then
                            for _, n in ipairs(energyNames) do
                                if hum:GetAttribute(n) ~= nil then pcall(function() hum:SetAttribute(n, 100) end) end
                            end
                        end
                        for _, v in ipairs(char:GetDescendants()) do
                            if v:IsA("NumberValue") or v:IsA("IntValue") then
                                local nl = v.Name:lower()
                                for _, n in ipairs(energyNames) do
                                    if nl:find(n:lower()) then pcall(function() v.Value = 100 end); break end
                                end
                            end
                        end
                    end
                    for _, n in ipairs({"Energy","Stamina","Sanity","Mental","Fear","SP","Endurance","Breath"}) do
                        if LocalPlayer:GetAttribute(n) ~= nil then pcall(function() LocalPlayer:SetAttribute(n, 100) end) end
                    end
                end
            end)
        end
    ...
    Callback = function(val)
        ...
        if val then
            ...
            for _, eff in ipairs(Lighting:GetChildren()) do
                ...
                or eff:IsA("DepthOfFieldEffect") then eff.Enabled = false end
            end
        else
            ...
            for _, eff in ipairs(Lighting:GetChildren()) do
                if eff:IsA("BlurEffect") or eff:IsA("ColorCorrectionEffect")
                or eff:IsA("BlackAndWhiteEffect") or eff:IsA("SunRaysEffect")
                or eff:IsA("DepthOfFieldEffect") then eff.Enabled = true end
            end
        end
    end,
})
...
VisualsTab:CreateSlider({
    Name = "Brightness Level", Range = {1, 20}, Increment = 1, Suffix = "x", CurrentValue = 2,
    Flag = "BrightnessLevel",
    Callback = function(val) Lighting.Brightness = val end,
})

VisualsTab:CreateToggle({
    Name = "🌫️ Remove Fog", CurrentValue = false, Flag = "RemoveFog",
    Callback = function(val)
        State.RemoveFog = val
...
VisualsTab:CreateToggle({
    Name = "🎨 Remove Post-Processing Effects", CurrentValue = false, Flag = "RemovePostFX",
    Callback = function(val)
...
VisualsTab:CreateSlider({
    Name = "Field of View", Range = {70, 120}, Increment = 5, Suffix = "°", CurrentValue = 70,
    Flag = "FOVSlider",
...
HuntTab:CreateToggle({
    Name = "🔔 Hunt Warning Notification", CurrentValue = false, Flag = "HuntWarning",
    Callback = function(val)
...
HuntTab:CreateToggle({
    Name = "🍆 Anti Sex (Auto Hunt Escape)", CurrentValue = false, Flag = "AntiSex",
    Callback = function(val)
        State.AntiSex = val
        if val then
            ...
            Rayfield:Notify({ Title = "🍆 Anti Sex DISABLED", Content = "You're on your own now.", Duration = 3, Image = 4483362458 })
        end
    end,
})
...
HuntTab:CreateButton({
    ...
    Callback = function() antiSexEscape() end,
})
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if savedWS and savedWS ~= 16 then Humanoid.WalkSpeed = savedWS end
end)
...
-- ============================================================
Rayfield:Notify({
    Title    = "✨ Astra Hub v3.0",
    Content  = "Загружено! 23 призрака · 8 улик\n🤖 Auto Play доступен во вкладке Evidence",
    Duration = 6,
    Image    = 4483362458,
})
